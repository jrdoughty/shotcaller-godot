...
func _ready():
	...
	for leader in autoload.leaders:
		var button = leader_select_button.instantiate()
		button.prepare(leader)
		leader_grid.add_child(button)
		button.connect("leader_selected",Callable(self,"_leader_selected"))
	var random_button = leader_select_button.instantiate()
	random_button.prepare("random")
	leader_grid.add_child(random_button)
	random_button.connect("leader_selected",Callable(self,"_leader_selected"))
